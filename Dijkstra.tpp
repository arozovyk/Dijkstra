...
std::set<std::pair<W, Vertex<W, T> *> *> * Dijkstra<W, T>::exec(Vertex<W, T> &S) {

    auto *resultSet = new std::set<std::pair<W, Vertex<W, T> *> *>();
    AdjacencyList<W, Vertex<W, T> *> *adjList = S.getList();
    int adjSize = adjList->getSize();
    for (int i = 0; i < adjSize; ++i) {
        std::pair<W, Vertex<W, T> *> *adjNode = adjList->getEl(i);
    ...
    for (auto it = vertices->begin(); it != vertices->end(); ++it) {
        ...
        if (!adjList->containsVertex((*it)) && !((*(*it)) == S)) {
            heap->insert(new std::pair<W, Vertex<W, T> *>(INT32_MAX, (*it)));
        }
    }
    ...
    while (res_set_size != graph_v_size - 1) {
        ...
        std::pair<W, Vertex<W, T> *> *rootMin = heap->get_root();
        adjList = rootMin->second->getList();

        for (int i = 0; i < adjList->getSize(); ++i) {
            std::pair<W,Vertex<W, T>*> *adj_pair_v_dist = adjList->getEl(i);
            ...
            if (!((*adj_pair_v_dist->second) == S) && !pair_is_in_result) {
                ...
                std::pair<W,Vertex<W, T>*> *heapPair=heap->getPair(index_heap);
